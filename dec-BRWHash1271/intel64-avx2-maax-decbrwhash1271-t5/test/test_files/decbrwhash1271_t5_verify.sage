...
def getKeyRandom(k):
	...
	key = []
	f = open("decbrwhash1271_t5_verify_keyfile.txt", "w")
	for i in range(k): 
...
def decimatedBRW(t,msgLen,keyFldElt,msgEltsLst):
	...
	for i in range(0,ell):
		tmpLst.append(msgEltsLst[i])
	for i in range(1,m-ell+1):
		tmpLst.append(F(0))
	tmpLst1 = []
	tmpLst2 = []
	tmpLst3 = []
	tmpLst4 = []
	for i in range(0,frakn):
		tmpLst1.append(tmpLst[4*i])
		tmpLst2.append(tmpLst[4*i+1])
		tmpLst3.append(tmpLst[4*i+2])
		tmpLst4.append(tmpLst[4*i+3])

	Q1 = ComputeBRW(t, keyFldElt, tmpLst1)
	Q2 = ComputeBRW(t, keyFldElt, tmpLst2)
	Q3 = ComputeBRW(t, keyFldElt, tmpLst3)
	Q4 = ComputeBRW(t, keyFldElt, tmpLst4)
	
	d = 1 + ZZ(floor(log(frakn,2)))
	gamma = keyFldElt^(2^d)
	Q = Q1*gamma^3 + Q2*gamma^2 + Q3*gamma + Q4
	
	val = keyFldElt^2*Q + keyFldElt*F(msgLen)
	return val	
...
def genTestCases():
	...
	t = 5;
	f1 = open("decbrwhash1271_t5_verify_test_cases.txt", "w")
	f2 = open("decbrwhash1271_t5_verify_s_outputs.txt", "w")
	start = 1
	stop = 2040
	step = 1
	for msgLen in range(start,stop,step):	
		for i in range(1,2):
			...
			f1.write("%d"%msgLen)
			f1.write("\n")
			digest = decimatedBRW(t,msgLen,keyFldElt,msgEltsLst) 
