...
int main() {
	...
	FILE *fpin,*fpout;
    	...
    	// #define SAGE
    	#ifdef SAGE    	
    	c = system("cd ./test_files; sage decbrwhash1305_t2_speed_record_test_cases.sage; cd ..");
    	remove("./test_files/decbrwhash1305_t2_speed_record_test_cases.sage.py");
    	#endif
    	
	format_key(k,KEY_LENGTH,"./test_files/decbrwhash1305_t2_speed_record_test_cases_keyfile.txt");
	q = (uint64 *)k; kp[0][0] = q[0]; kp[0][1] = q[1];
	
	line = calloc(16500,(sizeof(char)));
	msg  = calloc(16500,(sizeof(char)));	
    	
   	fpin = fopen("./test_files/decbrwhash1305_t2_speed_record_test_cases.txt", "r");
    	if (fpin == NULL) {
	...
	fpout = fopen("./test_files/decbrwhash1305_t2_speed_records.txt","w");
   	...
   	fpin = fopen("./test_files/decbrwhash1305_t2_speed_record_test_cases_direct_cmp.txt", "r");
    	if (fpin == NULL) {
	...
	fpout = fopen("./test_files/decbrwhash1305_t2_speed_records_direct_cmp.txt","w");
